...
class FacultadesApplicationTests {
	...
	private static final String URL_FACU_EDUCA="https://upload.wikimedia.org/wikipedia/commons/thumb/f/fd/Facultad_de_Derecho_%288617641510%29.jpg/1280px-Facultad_de_Derecho_%288617641510%29.jpg";
	private static final String URL_FACU_ARQUITEC="https://upload.wikimedia.org/wikipedia/commons/thumb/8/82/Facultad_de_arquitectura_urbanismo_y_diseno_UNC.jpg/800px-Facultad_de_arquitectura_urbanismo_y_diseno_UNC.jpg";
	@Autowired
...
//		Facultades newFacultad = new Facultades(null, "Facultad de Psicología", "Quito", "LaPiedra",
//				"https://www.ecured.cu/images/d/d6/Universidad_Central_del_Ecuador.jpg", 9L);
...
//	}
	...
	@Test
	@DirtiesContext
	void deberiaCrearNuevaFacultadYBorrarLaMisma() {
		Facultades newFacultad = new Facultades(null, "Facultad de Psicología", "Quito", "LaPiedra",
				"https://www.ecured.cu/images/d/d6/Universidad_Central_del_Ecuador.jpg", 9L);
		ResponseEntity<Void> createResponse = restTemplate.postForEntity("/facultades", newFacultad, Void.class);
		assertThat(createResponse.getStatusCode()).isEqualTo(HttpStatus.CREATED);
		...
		// borrar la misma Facultad
		ResponseEntity<Void> response = restTemplate.exchange("/facultades/"+id, HttpMethod.DELETE, null, Void.class);
		assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NO_CONTENT);
		ResponseEntity<String> getRespuesta = restTemplate.getForEntity("/facultades/"+id, String.class);
		assertThat(getRespuesta.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
	}
	...
	void deberiaDevolverTodasLasFacultadesCuandoSoliciteLista() {
		...
		assertThat(ids).containsExactlyInAnyOrder(1, 2, 52, 1202);

		JSONArray nombres = documentContext.read("$..nombre");
		assertThat(nombres).containsExactlyInAnyOrder("Facultad de Derecho", "Facultad de Odontología",
				"Facultad de Educación inicial", "Facultad de Arquitectura");

		JSONArray ciudades = documentContext.read("$..ciudad");
		assertThat(ciudades).containsExactlyInAnyOrder("México", "Bogota", "Cuzco", "San Salvador");

		JSONArray campuses = documentContext.read("$..campus");
		assertThat(campuses).containsExactlyInAnyOrder("JurisPraeceptum", "Colgate", "EDU", "UCA");

		JSONArray photos = documentContext.read("$..photo");
		assertThat(photos).containsExactlyInAnyOrder(URL_FACU_DERECHO, URL_FACU_ODONTOLOGIA, URL_FACU_EDUCA,
				URL_FACU_ARQUITEC);

		JSONArray carrerass = documentContext.read("$..carreras");
		assertThat(carrerass).containsExactlyInAnyOrder(4, 4, 9, 5);
	...
	void deberiaActualizarUnaFacultadExistente() {
		Facultades facultadUpdate = new Facultades(null, "Facultad de Educación inicial", "Cuzco", "EDU",
				URL_FACU_DERECHO, 9L);
	...
	void noDeberiaActualizarUnaFacultadQueNoExiste() {
		Facultades facultadDesconocida = new Facultades(null, "Facultad Química", "Sucre", "Quimic",
				"htt:ww/sccb/jede.jpg", 5L);
